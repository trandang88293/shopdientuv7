...
public class AccountStaffController {
    ...
    // ✅ Lấy danh sách nhân viên với tìm kiếm theo tên và username có phân trang
    @GetMapping
    public ResponseEntity<?> getEmployees(
        ...
        // Giả sử role = 3 là nhân viên
        if (!keyword.isEmpty()) {
            accounts = accountRepository.searchByNameOrUsername((byte) 3, keyword, pageable);
        } else {
            accounts = accountRepository.findByRole((byte) 3, pageable);
        }
        return ResponseEntity.ok(accounts);
    }

    // ✅ Thêm nhân viên với ảnh đại diện (upload lên Cloudinary và mã hóa mật khẩu)
    @PostMapping
    public ResponseEntity<?> createAccount(
        ...
        try {
            ...
            // Nếu có file avatar, upload lên Cloudinary và lấy URL trả về
    ...
    // ✅ Cập nhật nhân viên (upload ảnh mới lên Cloudinary nếu có thay đổi và mã hóa
    // mật khẩu)
    @PutMapping("/{id}")
    public ResponseEntity<?> updateAccount(
        ...
        try {
            // Kiểm tra trùng lặp username và email đối với các tài khoản khác
